...
If `Brain/`, `ideas/`, and `memory/` are checked out in the workspace, this workspace is
linked to Brain — a gateway to two shared knowledge repos. Follow `Brain/PROTOCOL.md` as
...
3. Query `ideas` (mining mode, per `ideas/README.md`): search `ideas/idea-repository.md`
   by domain + keyword; surface relevant fragments, including cross-domain ones, each with
   a one-line "how it applies".
